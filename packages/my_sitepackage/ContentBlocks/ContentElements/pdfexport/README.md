...
## 13. Puppeteer Chrome-Binary im Produktiv-Container (Bugfix)

**Problem:** Lokal funktionierte die PDF-Generierung, auf dem Live-Server kam jedoch ein `Oops, an error occurred!`-Fehler.

**Ursache:** Beim Docker-Build lädt `npm ci` das Puppeteer-eigene Chrome-Binary **nicht** nach `node_modules/`, sondern nach `~/.cache/puppeteer/` im Build-Stage. Da in den Final-Stage nur `node_modules` kopiert wurde, fehlte das Binary im Container. Das systemseitig installierte `chromium`-Paket war zwar vorhanden, aber weder Puppeteer noch Browsershot wussten, wo es zu finden ist.

**Lösung (drei Änderungen):**

1. **`Dockerfile` – vite-builder Stage:** `PUPPETEER_SKIP_DOWNLOAD=true` verhindert den nutzlosen Chrome-Download beim Build, da der Container das System-Chromium verwendet.
...
2. **`Dockerfile` – Final-Stage:** `PUPPETEER_EXECUTABLE_PATH` zeigt auf das installierte System-Chromium, damit Puppeteer es automatisch findet.
